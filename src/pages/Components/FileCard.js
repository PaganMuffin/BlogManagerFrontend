...
export const FileCard = ({data, updateFile}) => {
    ...
    return (
        <div class=" rounded-lg border shadow-md bg-slate-800 ">
        <div className="flex justify-center items-center aspect-video bg-white bg-opacity-20">
            { img_ext.includes(ext) ? 
                <img class="rounded-t-lg h-full " src={`${process.env.REACT_APP_API_URL}/file/${data.filename}`} alt="" />
                :
                <img class="rounded-t-lg h-full " src={`https://www.salonlfc.com/wp-content/uploads/2018/01/image-not-found-scaled-1150x647.png`} alt="" />
            }
            
        </div>
        <p class=" px-2 tracking-tight text-white truncate">{data.filename}</p>
